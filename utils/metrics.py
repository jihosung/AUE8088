...
# new ver(GPT).
# def bbox_ioa(box1, box2, xywh=True, eps=1e-7):
#     """
#     Calculates IoA (Intersection over Area of box1) between two sets of boxes.

#     Args:
#         box1 (Tensor[N, 4]): predicted boxes
#         box2 (Tensor[M, 4]): ground truth crowd boxes
#         xywh (bool): if True, convert box1 and box2 from (x, y, w, h) to (x1, y1, x2, y2)
#     Returns:
#         Tensor[N, M]: pairwise IoA values
#     """
#     if xywh:
#         (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
#         w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
...
#     inter = (b1_x2[:, None].minimum(b2_x2) - b1_x1[:, None].maximum(b2_x1)).clamp(0) * \
#             (b1_y2[:, None].minimum(b2_y2) - b1_y1[:, None].maximum(b2_y1)).clamp(0)

#     area1 = (b1_x2 - b1_x1) * (b1_y2 - b1_y1) + eps  # [N, 1]
#     return inter / area1[:, None]  # [N, M]

# new ver.(handmade)
def bbox_ioa(box1, box2, xywh=True, eps=1e-7):
    """
    Calculates IoA between two boxes, supporting xywh/xyxy formats.
