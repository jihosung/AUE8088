...
def random_perspective_rgbt(
    im, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0)
):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(0.1, 0.1), scale=(0.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    (im_lwir, im_vis) = im
    height = im_lwir.shape[0] + border[0] * 2  # shape(h,w,c)
    width = im_lwir.shape[1] + border[1] * 2
    # print("[random_perspetive] h, w: ", height, width)

    # Center
    C = np.eye(3)
    C[0, 2] = -im_lwir.shape[1] / 2  # x translation (pixels)
    C[1, 2] = -im_lwir.shape[0] / 2  # y translation (pixels)
    ...
    # Combined rotation matrix, 똑같은 M matrix lwir, vis 동시 적용
    M = T @ S @ R @ P @ C  # order of operations (right to left) is IMPORTANT
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            im_lwir = cv2.warpPerspective(im_lwir, M, dsize=(width, height), borderValue=(114, 114, 114))
            im_vis = cv2.warpPerspective(im_vis, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            im_lwir = cv2.warpAffine(im_lwir, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            im_vis = cv2.warpAffine(im_vis, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
    im = (im_lwir, im_vis)
...
def mixup_rgbt(imgs, labels, imgs2, labels2):
    ...
    (img2_lwir, img2_vis) = imgs2
    img_lwir = (img_lwir * r + img2_lwir * (1 - r)).astype(np.uint8)
    img_vis = (img_vis * r + img2_vis * (1 - r)).astype(np.uint8)
    labels = np.concatenate((labels, labels2), 0)
