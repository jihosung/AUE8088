...
class LoadRGBTImagesAndLabels(LoadImagesAndLabels):
    ...
    def __init__(self, path, **kwargs):
        ...
        is_train = 'train' in str(path).strip()
        ...
        print("[dataloaders] is_train:", is_train)
        print("[dataloaders] augment", self.augment)
        self.mosaic = (is_train == True) and (self.augment == True) # train에다가 augment가 true이면 mosaic on
        print("[dataloaders] mosaic:", self.mosaic)
    ...
    # TODO: make this function work on rgbt image
    def load_mosaic_rgbt(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            (img_lwir, img_vis) = img # separate image


            # place img in img4
            if i == 0:  # top left
                img4_lwir = np.full((s * 2, s * 2, img_lwir.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                img4_vis = np.full((s * 2, s * 2, img_vis.shape[2]), 114, dtype=np.uint8)
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
            elif i == 1:  # top right
                x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
            ...
            # Labels
            """
            여기 self.labels에는 xywh normalized 형식이 들어있다. 다만, xy가 lefttop 기준
            하지만 아래에서 다루는 xywhn 형식은 xy가 center 좌표이다
            따라서 변환이 필요함
            """
            labels, segments = self.labels[index].copy(), self.segments[index].copy()
            if labels.size:
        ...
        labels4 = np.concatenate(labels4, 0)
        for x in (labels4[:, 1:], *segments4):
            np.clip(x, 0, 2 * s, out=x)  # clip when using random_perspective(). x를 0~2*s 범위로 자른다.
        # img4, labels4 = replicate(img4, labels4)  # replicate
        # print("labels4 before random perspective:", labels4)
        # Augment
        # no segment information in KAIST-RGBT dataset
        # img4, labels4, segments4 = copy_paste(img4, labels4, segments4, p=self.hyp["copy_paste"])

        # random perspective
        img4 = (img4_lwir, img4_vis)
        img4, labels4 = random_perspective_rgbt(
            img4,
            labels4,
            segments4,
            degrees=self.hyp["degrees"],
        ...
        # print("labels4 after random perspective:", labels4) # 640 640 기준

        return img4, labels4 # xyxy

    """
    이거는 새로 시도했던 getitem 코드
    mosaic else: 에서 for문을 풀어버리니까 evaluation이 잘 안됨. validation 과정에서 문제가 생기는것 같다.
    """
    # def __getitem__(self, index):
    #     """Fetches the dataset item at the given index, considering linear, shuffled, or weighted sampling."""
    #     index = self.indices[index]  # linear, shuffled, or image_weights
    ...
    #     hyp = self.hyp
    #     mosaic = self.mosaic and random.random() < hyp["mosaic"]
    #     # 1. mosaic 적용 or 적용x / output: xyxy labels ===================================================================
    #     if mosaic:
    ...
    #         # log
    #         # print("\x1b[s mosaic applied. \x1b[u", end='', flush=True)
    #         # print("[mosaic] applied!")

    #         (img_lwir, img_vis), labels = self.load_mosaic_rgbt(index) # labels: xyxy format
    #         # print("mosaic img_lwir shape:", img_lwir.shape)
    #         shapes = None
    ...
    #     else: # mosaic 적용 안할때
    #         # print("[mosaic] no mosaic!")
    #         # Load image
    #         # hw0s: original shapes, hw1s: resized shapes
    #         (img_lwir, img_vis), hw0s, hw1s = self.load_image(index)
    #         (h0, w0) = hw0s[0]
    #         (h, w)= hw1s[0]
    ...
    #         #     # TODO: 여기도 random_perspective_rgbt로 바꾸기 -> 큰 for문 없애야할듯?
    #         #     img, labels = random_perspective(
    #         #         img,
    #         #         labels,
    #         #         degrees=hyp["degrees"],
    ...
    #     # 2. 공통 augment 적용 ===================================================================
    #     # TODO: mosaic 했으면 아래 공통 augment들은 안하는게 효과적인지? 아니면 공통으로 묶는게 효과적인지?
    #     nl = len(labels)  # number of labels
    #     if nl:
    #         labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=img_lwir.shape[1], h=img_lwir.shape[0], clip=True, eps=1e-3) # 라벨 변환
    ...
    #     return (img_lwir, img_vis), labels_out, self.im_files[index], shapes, index
    
    """
    이거는 실제로 mosaic를 써서 성능이 올랐던 getitem 코드
    mosaic else: 부분 안건드리고 if mosaic: 부분만 건드림.
    if mosaic: 를 거친 데이터들이 output이랑 동일하도록 형식만 맞춰줌.
    """
    def __getitem__(self, index):
        """Fetches the dataset item at the given index, considering linear, shuffled, or weighted sampling."""
        index = self.indices[index]  # linear, shuffled, or image_weights
        ...
        if mosaic:
            ...
            for ii, (img, (h0, w0), (h, w)) in enumerate(zip(imgs, hw0s, hw1s)):
                ...
                # TODO: 여기있는 augmentation 작동하게 만들기
                # if self.augment:
                #     raise NotImplementedError('Please make data augmentation work!')
